...
namespace Sibe.API.Services.EmailService
{
    public class EmailService : IEmailService
    {
        ...
        private readonly string _footerPath = "wwwroot/html/footer.html"; // Ruta al pie de pagina
        ...
        private MimeMessage CreateMessage(string toEmail, string subject, string body, string? attachmentPath, byte[]? attachment, string? attachmentName)
        {
            ...
            // Leer el HTML del footer desde el archivo
            string footerHtml;
            string footerTemplatePath = Path.Combine(Directory.GetCurrentDirectory(), _footerPath);
            using (StreamReader reader = new StreamReader(footerTemplatePath))
            {
                footerHtml = reader.ReadToEnd();
            }

            // Concatenar el cuerpo del mensaje con el footer
            bodyBuilder.HtmlBody = $"{body}{footerHtml}";
